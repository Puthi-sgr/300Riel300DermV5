...
const UniqueReachCard: React.FC<Props> = ({ uniqueReach, totalViews, t }) => {
  const progress = totalViews?.value
    ? Math.min(100, Math.round(((uniqueReach?.value ?? 0) / totalViews.value) * 100))
    : 0;

  return (
    <article className="rounded-[24px] bg-card-fern border border-white/70 shadow-impact p-6 space-y-4 text-earth-900">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-2 text-3xl font-bold">{formatNumber(uniqueReach?.value)}</div>
        </div>
