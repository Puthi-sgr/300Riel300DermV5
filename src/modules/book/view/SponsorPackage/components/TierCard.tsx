...
const TierCard: React.FC<TierCardProps> = ({
  ...
  >
    {glowBackdropClass ? (
      <div className={`absolute inset-0 pointer-events-none ${glowBackdropClass}`} aria-hidden />
    ) : null}
    <div className="relative space-y-4">
   
      <div className="flex items-center justify-between">
        <div className={`h-10 w-10 rounded-full flex items-center justify-center shadow-sm border border-white/70 ${pillClass}`}>
          {icon}
        </div>
        <span className={`text-xs font-semibold px-3 py-1 rounded-full border border-eco-100 ${badgeClass}`}>
          {tierLabel}
        </span>
      </div>
      ...
      <div className="space-y-1">
        ...
        <p className="text-3xl font-extrabold text-earth-900">
          {price} <span className="text-base font-semibold text-earth-700">/ {period}</span>
        </p>
      </div>
      ...
      <div>
        <FullRoundedGradientButton className={`w-full justify-center rounded-xl ${ctaClass}`}>
          {ctaLabel}
